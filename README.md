...
- 국가별 / 토픽별 통계 차트
- **국가 × 토픽 히트맵** (15개국 × 20토픽, Row% / Col% / Absolute 모드)
- **유사 논문 추천** — SPECTER2 임베딩 기반 top-5 (논문 카드 펼치면 하단)
- 헤더(`ICRA 2026 · Paper Explorer`) 클릭 → 모든 필터 리셋 후 홈 복귀
...
output/papers.json                         ───► scripts/embed_papers.py      ─► classification/similar_papers.json
                                                                                  (SPECTER2 임베딩 + top-K 코사인 유사도, MPS GPU 사용)
...
python3 scripts/embed_papers.py        # 5) SPECTER2 임베딩 + top-10 유사논문 사전계산
                                       #    pip install torch transformers numpy 필요
                                       #    Apple Silicon은 MPS GPU 자동 사용 (~2분)
python3 scripts/build_html.py          # 6) 최종 HTML 생성
